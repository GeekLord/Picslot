...
const App: React.FC = () => {
  ...
  // Effect to disable compare mode when switching tabs
  useEffect(() => {
    if (isCompareMode) {
  ...
  const handleStudioPortrait = useCallback(async () => {
    if (!currentImage) {
      setError('No image loaded to create a portrait from.');
      return;
    }
    ...
    try {
        const portraitImageUrl = await generateStudioPortrait(currentImage);
        const newImageFile = dataURLtoFile(portraitImageUrl, `portrait-${Date.now()}.png`);
        addImageToHistory(newImageFile);
    } catch (err) {
        const errorMessage = err instanceof Error ? err.message : 'An unknown error occurred.';
    ...
    return (
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center gap-6 animate-fade-in">
        <div className="relative w-full shadow-2xl rounded-xl overflow-hidden bg-black/20">
            ...
            {displayHotspot && !isLoading && activeTab === 'retouch' && !isCompareMode && (
                <div 
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 mt-6">
            ...
            {canUndo && (
                <button
                    ...
                    className={`flex items-center justify-center text-center border font-semibold py-3 px-5 rounded-md transition-all duration-200 ease-in-out active:scale-95 text-base
                        ...
                        disabled:opacity-50 disabled:cursor-not-allowed disabled:bg-white/5 disabled:border-white/20 disabled:shadow-none`}
                    aria-label={isCompareMode ? "Exit compare mode" : "Compare with original"}
                >
            ...
            <button
                ...
                className="flex items-center justify-center text-center bg-gradient-to-br from-purple-600 to-pink-500 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-purple-500/20 hover:shadow-xl hover:shadow-purple-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base disabled:opacity-50 disabled:cursor-not-allowed disabled:from-purple-800 disabled:to-pink-700 disabled:shadow-none disabled:transform-none"
            ...
            <button
                ...
                disabled={!currentImage || isLoading}
                className="flex items-center justify-center text-center bg-gradient-to-br from-amber-500 to-orange-500 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-amber-500/20 hover:shadow-xl hover:shadow-amber-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base disabled:opacity-50 disabled:cursor-not-allowed disabled:from-amber-700 disabled:to-orange-600 disabled:shadow-none disabled:transform-none"
            ...
            <button
                onClick={handleStudioPortrait}
                disabled={!currentImage || isLoading}
                className="flex items-center justify-center text-center bg-gradient-to-br from-teal-500 to-cyan-500 text-white font-bold py-3 px-5 rounded-md transition-all duration-300 ease-in-out shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/40 hover:-translate-y-px active:scale-95 active:shadow-inner text-base disabled:opacity-50 disabled:cursor-not-allowed disabled:from-teal-700 disabled:to-cyan-600 disabled:shadow-none disabled:transform-none"
                aria-label="Create studio portrait"
            >
